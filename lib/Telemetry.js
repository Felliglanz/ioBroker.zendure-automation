...
const HISTORY_SAMPLE_MS = 5 * 60 * 1000; // one averaged power point every 5min, for the dashboard's rolling graphs
...
class Telemetry {
    constructor(adapter) {
        ...
        // Running sum/count per history metric, accumulated every recordCycle() call and flushed
        // into one averaged history point every HISTORY_SAMPLE_MS - a handful of numbers in RAM,
        // not a growing buffer, so the 5min resolution reflects the whole interval instead of
        // whatever the power happened to be at the exact instant the timer ticked over.
        this._historyAccum = this._freshHistoryAccum();
    ...
    _accumulate(key, value) {
        ...
        this._historyAccum[key].sum += value;
        this._historyAccum[key].count += 1;
    }

    _freshHistoryAccum() {
        return {
            houseW: { sum: 0, count: 0 },
            gridW: { sum: 0, count: 0 },
            pvW: { sum: 0, count: 0 },
            batteryW: { sum: 0, count: 0 }
        };
    }
    ...
    async recordCycle({ gridPowerW, batteryPowerW, pvPowerW, houseW, emergencyActive }) {
        ...
        // Feed this cycle's readings into the running per-metric average, then only flush an
        // actual history point (and reset the accumulator) once HISTORY_SAMPLE_MS has elapsed -
        // so each point represents the whole interval, not just its last instant.
        this._accumulate('houseW', houseW);
        this._accumulate('gridW', gridPowerW);
        this._accumulate('pvW', pvPowerW);
        this._accumulate('batteryW', batteryPowerW);

        if (this._lastHistorySampleMs === null || nowMs - this._lastHistorySampleMs >= HISTORY_SAMPLE_MS) {
            this._lastHistorySampleMs = nowMs;
            const avg = key => {
                const { sum, count } = this._historyAccum[key];
