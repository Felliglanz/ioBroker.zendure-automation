...
class DataReader {
     ...
     * @param {boolean} [ignoreStateFreshness] - Skip the frozen-state check (legacy/non-ZenSDK devices)
     * @returns {Promise<number|null>} SOC in percent (0-100)
     */
    async getBatterySoc(ignoreStateFreshness = false) {
        try {
            const state = await this.adapter.getForeignStateAsync(
                `${this.deviceBasePath}.electricLevel`
            );
            if (state && state.val !== null && state.val !== undefined) {
                if (!ignoreStateFreshness && isStale(state)) {
                    this.adapter.log.warn('Battery SOC has not been updated recently, ignoring stale value');
                    return null;
                }
                const value = Number(state.val);
                if (!isFinite(value)) {
                    this.adapter.log.warn(`Battery SOC is invalid (NaN/Infinity), ignoring value: ${state.val}`);
                    return null;
                }
                return value;
            }
     ...
     */
    async getCurrentBatteryPowerW(ignoreStateFreshness = false) {
        try {
            ...
            if (packPowerState && packPowerState.val !== null && packPowerState.val !== undefined) {
                if (!ignoreStateFreshness && isStale(packPowerState)) {
                    this.adapter.log.warn(
                        'Battery power (packPower) has not been updated recently - treating as unavailable ' +
                        'instead of trusting a frozen value (source adapter may be offline/unresponsive)'
                    );
                    return null;
                }
                const batteryPower = -Number(packPowerState.val);
                if (!isFinite(batteryPower)) {
                    this.adapter.log.warn(`Battery power is invalid (NaN/Infinity), using cached value: ${this._lastBatteryPowerW}W`);
                    return this._lastBatteryPowerW;
                }
    ...
    /**
     * Read grid input power from Zendure device (AC charging setpoint)
     * This is the power set via API, WITHOUT PV input contribution.
     * Use this for validation when device has PV modules attached.
     * Convention: Same as packPower - negative = charging, positive = discharging
     * @param {boolean} [ignoreStateFreshness] - Skip the frozen-state check (legacy/non-ZenSDK devices)
     * @returns {Promise<number|null>} Power in Watts
     */
    async getGridInputPowerW(ignoreStateFreshness = false) {
        try {
            // Note: gridInputPower also uses Zendure's inverted convention
            //   Zendure: negative=discharge, positive=charge
            //   Our code: positive=discharge, negative=charge
            // Therefore we invert the value (same as packPower)
            const gridInputState = await this.adapter.getForeignStateAsync(
                `${this.deviceBasePath}.gridInputPower`
            );

            if (gridInputState && gridInputState.val !== null && gridInputState.val !== undefined) {
                if (!ignoreStateFreshness && isStale(gridInputState)) {
                    this.adapter.log.warn('Grid input power has not been updated recently, ignoring stale value');
                    return null;
                }
                return -Number(gridInputState.val);
            }
        } catch (err) {
            this.adapter.log.warn(`Could not read grid input power: ${err.message}`);
        }
        return null;
    }
    ...
    async getMinimumPackVoltageV() {
        try {
            ...
            for (const id of Object.keys(allStates)) {
                if (id.endsWith('.minVol')) {
                    ...
                    if (state && state.val !== null && state.val !== undefined) {
                        ...
                        this.adapter.log.debug(`Pack ${id.split('.')[5]}: minVol = ${voltage.toFixed(2)}V`);
                    }
                }
            }
