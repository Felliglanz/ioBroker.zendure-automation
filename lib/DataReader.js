    ...
    async getGridPowerW(powerMeterDp) {
        try {
            ...
            if (state && state.val !== null && state.val !== undefined) {
                ...
                if (isNaN(value)) {
                    this.adapter.log.warn(`Grid power is NaN, ignoring value: ${state.val}`);
                    return null;
                }
                return value;
            }
    ...
    async getBatterySoc() {
        try {
            ...
            if (state && state.val !== null && state.val !== undefined) {
                const value = Number(state.val);
                if (isNaN(value)) {
                    this.adapter.log.warn(`Battery SOC is NaN, ignoring value: ${state.val}`);
                    return null;
                }
                return value;
            }
        ...
        try {
            ...
            if (packPowerState && packPowerState.val !== null && packPowerState.val !== undefined) {
                ...
                if (isNaN(batteryPower)) {
                    this.adapter.log.warn(`Battery power is NaN, using cached value: ${this._lastBatteryPowerW}W`);
