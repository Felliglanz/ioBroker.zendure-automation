...
class MultiDeviceManager {
        ...
        for (const device of availableDevices) {
            ...
            if (safetyLimiter && totalPowerW > 0) {
                ...
                const result = await safetyLimiter.applySafetyLimits({
                    ...
                    powerW: totalPowerW / eligibleDevices.length
                });
     ...
     * @param {number} eligibleDeviceCount - Number of participating devices
     * @param {object} config - Adapter configuration
     * @returns {number} Limited total power
     */
    limitPowerToEligibleDevices(totalPowerW, eligibleDeviceCount, config) {
        if (eligibleDeviceCount <= 0) {
            return 0;
        }

        const maxChargePowerW = Math.max(0, Number(config.maxChargePowerW ?? 1600));
        const maxDischargePowerW = Math.max(0, Number(config.maxDischargePowerW ?? 1600));
        const maxTotalPowerW = totalPowerW < 0
            ? -maxChargePowerW * eligibleDeviceCount
            : maxDischargePowerW * eligibleDeviceCount;
