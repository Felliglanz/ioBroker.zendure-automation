    ...
    async distributePower(totalPowerW, aggregatedState, config, emergencyManagers, safetyLimiters) {
        ...
        for (const device of availableDevices) {
            ...
            if (safetyLimiter && totalPowerW > 0) {
                ...
                const limitedPower = safetyLimiter.applySafetyLimits(
                    totalPowerW / eligibleDevices.length,  // Per-device share
                    device.soc,
                    device.minPackVoltageV
                );
                
                if (limitedPower === 0) {
                    const reason = safetyLimiter.getLastLimitReason() || 'Safety limit';
                    excludedDevices.push({
                        deviceId: device.id,
                        deviceName: device.name,
                        powerW: 0,
