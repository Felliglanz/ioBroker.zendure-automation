...
class MultiDeviceManager {
    ...
    async distributePower(totalPowerW, aggregatedState, config, emergencyManagers, safetyLimiters) {
        ...
        for (const device of availableDevices) {
            ...
            const deviceConfig = waterfillActive
                ? { ...config, minBatterySoc: device.minSoc, maxBatterySoc: device.maxSoc }
                : config;

            if (waterfillActive && totalPowerW < 0 && device.chargeAllowed === false) {
            ...
            if (waterfillActive && totalPowerW > 0 && device.dischargeAllowed === false) {
                excludedDevices.push({
                    deviceId: device.id,
                    deviceName: device.name,
                    powerW: 0,
            ...
            if (totalPowerW < 0) {
                ...
                const maxBatterySoc = deviceConfig.maxBatterySoc ?? 100;
                ...
                const protectionMode = deviceConfig.dischargeProtectionMode || 'soc';
                if (protectionMode === 'soc') {
                    const minBatterySoc = deviceConfig.minBatterySoc ?? 10;
