...
class MultiDeviceManager {
    ...
    async distributePower(totalPowerW, aggregatedState, config, emergencyManagers, safetyLimiters) {
        ...
        // Scale down total power proportionally when devices are excluded
        // Example: 2 devices, total -3600W → 1 excluded → scale to -1800W
        const scaledTotalPowerW = Math.round(
            totalPowerW * (eligibleDevices.length / availableDevices.length)
        );
        
        if (excludedDevices.length > 0) {
            this.adapter.log.debug(
                `Power scaling: ${excludedDevices.length} device(s) excluded, ` +
                `scaled power from ${totalPowerW}W to ${scaledTotalPowerW}W ` +
                `(${eligibleDevices.length}/${availableDevices.length} devices active)`
            );
        }

        // Equal split distribution with scaled power
