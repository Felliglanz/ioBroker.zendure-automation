...
class PowerRegulator {
     ...
     * @param {boolean} [params.bypassHysteresis] - Skip only the hysteresis step (ramping/limits/
     *   rounding still apply). Set this from RelayProtection's `relayModified` flag: it targets an
     *   exact setpoint (0W or ±operatingDeadbandW) to let the relay switch, which the hysteresis
     *   dead band would otherwise revert, deadlocking the mode switch (issue #21).
     * @returns {{powerW: number}} Regulated power value
     */
    applyRegulation(params) {
        const { config, powerW, lastSetPowerW, safetyActive, bypassHysteresis } = params;
        ...
        // Apply hysteresis, unless RelayProtection is enforcing an exact setpoint
        if (bypassHysteresis) {
            this.adapter.log.debug('Hysteresis bypassed (relay protection is enforcing an exact setpoint)');
        } else {
    ...
    _applyRamping(config, powerW, lastSetPowerW) {
        // Use explicit undefined check to allow 0 = disabled
        const rampChargeLimit = config.rampChargeWPerCycle !== undefined 
            ? config.rampChargeWPerCycle 
            : 100;
        const rampDischargeLimit = config.rampDischargeWPerCycle !== undefined 
            ? config.rampDischargeWPerCycle 
            : 400;

        // Ramping = 0 means disabled (infinite ramp rate)
        if (rampChargeLimit === 0 && rampDischargeLimit === 0) {
            this.adapter.log.debug('Ramping disabled (0), no rate limiting applied');
            return powerW;
        }
        ...
        if (isCurrentlyInDischargeMode) {
            // Currently discharging: use discharge ramp (skip if ramp = 0)
            if (rampDischargeLimit > 0 && Math.abs(powerChange) > rampDischargeLimit) {
                if (powerChange > 0) {
            ...
            // Currently charging: use charge ramp (skip if ramp = 0)
            if (rampChargeLimit > 0 && Math.abs(powerChange) > rampChargeLimit) {
                if (powerChange > 0) {
            ...
            if (rampDischargeLimit > 0 && powerChange > 0 && Math.abs(powerChange) > rampDischargeLimit) {
                // Starting discharge
                this.adapter.log.debug(`Starting discharge with ramp: ${rampDischargeLimit}W/cycle`);
                return rampDischargeLimit;
            } else if (rampChargeLimit > 0 && powerChange < 0 && Math.abs(powerChange) > rampChargeLimit) {
                // Starting charge
