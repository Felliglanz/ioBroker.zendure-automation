...
class MultiDeviceController {
    ...
    async runCycle(config) {
        // ========== VALIDATE PREVIOUS SETPOINT (NON-BLOCKING) ==========
        // Aggregate device states first to get total current power
        const aggregatedState = await this.multiDeviceMgr.aggregateDeviceStates();
        const totalCurrentPowerW = aggregatedState.totalPowerW;
        await this.validationService.validateSetpoint(config, totalCurrentPowerW);
