...
class MultiDeviceController {
    ...
    async runCycle(config) {
        ...
        const emergencyTotal = emergencyDevices.length * (-(config.emergencyChargePowerW || 800));
        this.lastTotalWrittenPowerW = totalWritten + emergencyTotal;
    ...
    async checkEmergencies(config, devices) {
        ...
        for (const device of devices) {
            ...
            } else if (emergencyMgr.inMinSocRecovery) {
                emergencyDevices.push(device);
                // Log only once when entering recovery
                const logState = this.deviceRecoveryLogged.get(device.id) || {};
                if (!logState.minSoc) {
                    this.adapter.log.warn(`⚡ ${device.name} minSoc recovery active (Zendure hardware protection)`);
                    this.deviceRecoveryLogged.set(device.id, { ...logState, minSoc: true });
                }
            } else {
    ...
    async handleEmergencyDevices(config, emergencyDevices) {
        ...
        for (const device of emergencyDevices) {
            ...
            if (deviceConfig) {
                await this.validationService.writePowerSetpoint(device.id, deviceConfig.basePath, emergencyChargePower);
            }
        }
    }
    ...
    async calculateTargetPower(config, filteredGridPowerW, targetGridPowerW, normalDevices, aggregatedState) {
        ...
        const waterfillLimits = waterfillActive
            ? this.getWaterfillSystemLimits(normalDevices, config)
            : null;
    ...
    getWaterfillSystemLimits(normalDevices) {
        ...
        return {
            maxChargePowerW: configuredDevices.reduce(
                (sum, device) => sum + Math.max(0, Number(device.maxChargePowerW) || 0),
                0
            ),
            maxDischargePowerW: configuredDevices.reduce(
                (sum, device) => sum + Math.max(0, Number(device.maxDischargePowerW) || 0),
                0
            )
        };
    }
