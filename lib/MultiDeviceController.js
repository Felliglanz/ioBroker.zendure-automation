...
class MultiDeviceController {
    ...
    async runCycle(config) {
        ...
        for (const deviceState of aggregatedState.devices) {
            ...
            // Find device config to get validationSource
            const deviceConfig = config.devices?.find(d => 
                `${config.zendureSolarflowInstance}.${d.productKey}.${d.deviceKey}` === deviceState.basePath
            );
            ...
            if (validationPowerW !== null) {
                await this.validationService.validateSetpoint(deviceState.id, config, validationPowerW, deviceState.soc);
            }

            // Always checked against real battery power (deviceState.powerW), independent
            // of this device's configured validationSource above - same reasoning as
            // SingleDeviceController: "did we actually reach idle" has no other source.
            await this.validationService.validateZeroSetpoint(deviceState.id, deviceState.basePath, config, deviceState.powerW);
        }
    ...
    async checkEmergencies(config, devices) {
        ...
        for (const device of devices) {
            ...
            await emergencyMgr.updateMaxSocRecovery(config, device.soc);
    ...
    async handleEmergencyDevices(config, emergencyDevices) {
        ...
        for (const device of emergencyDevices) {
            ...
            if (deviceConfig) {
                const powerW = -this.getCappedEmergencyChargePowerW(deviceConfig, globalEmergencyChargePowerW);
     ...
     * maxChargePowerW so a large global emergencyChargePowerW can't overshoot
     * a smaller device's limit. Shared by handleEmergencyDevices (which writes
     * the setpoint) and createFullDistribution (which reports it in status).
     */
    getCappedEmergencyChargePowerW(deviceConfig, globalEmergencyChargePowerW) {
        const deviceLimitW = Number(deviceConfig.maxChargePowerW);
        return Number.isFinite(deviceLimitW) && deviceLimitW > 0
    ...
    async calculateTargetPower(config, filteredGridPowerW, targetGridPowerW, normalDevices, aggregatedState) {
        ...
        const regulatorGain = config.regulatorGainEnabled && Number.isFinite(config.regulatorGain)
            ...
            : 1;
        let newTotalBatteryPowerW = lastSetPowerW + regulatorGain * (filteredGridPowerW - targetGridPowerW);
        ...
        // Mirrored on the charge side (e.g. every device at maxBatterySoc while PV
        // surplus keeps requesting charge). chargeAllowed lives on the configured
        // device list (this.multiDeviceMgr.devices), not the raw telemetry in
        // normalDevices, so look it up there.
        const allNormalDevicesChargeBlocked = normalDevices.every(d => {
            const configuredDevice = this.multiDeviceMgr.devices.find(cd => cd.id === d.id);
            const chargeAllowed = configuredDevice ? configuredDevice.chargeAllowed !== false : true;
            const em = this.emergencyManagers?.get(d.id);
            const inMaxSocRecovery = Boolean(em) && em.inMaxSocRecovery;
            return !chargeAllowed || inMaxSocRecovery || (Number.isFinite(d.soc) && d.soc >= (config.maxBatterySoc ?? 100));
        });
        const chargeBlocked = !config.enableCharge || allNormalDevicesChargeBlocked;
        ...
        const multiDeviceConfig = {
            ...
            operatingDeadbandW: (config.operatingDeadbandW || 10) * totalDevicesCount,
            // PowerRegulator's hysteresis compares against newTotalBatteryPowerW, which already
            // has regulatorGain applied - without this scaling, a lower gain would silently
            // inflate the effective grid-error tolerance to hysteresisW/gain instead of the
            // configured hysteresisW (issue #30: gain 0.4 turned a 50W hysteresis into a 125W
            // dead zone, causing the regulator to get stuck well away from target).
            hysteresisW: (config.hysteresisW || 50) * regulatorGain
        };
    ...
    getWaterfillSystemLimits(normalDevices) {
        const configuredDevices = normalDevices
            .map(device => this.multiDeviceMgr.devices.find(configured => configured.id === device.id))
            .filter(Boolean);

        return {
            maxChargePowerW: configuredDevices.reduce(
                (sum, device) => sum + (device.chargeAllowed === false ? 0 : Math.max(0, Number(device.maxChargePowerW) || 0)),
                0
            ),
            maxDischargePowerW: configuredDevices.reduce(
    ...
    createFullDistribution(config, emergencyDevices, normalDistribution) {
        ...
        const emergencyDistribution = emergencyDevices.map(d => {
            ...
            const powerW = deviceConfig
                ? -this.getCappedEmergencyChargePowerW(deviceConfig, globalEmergencyChargePowerW)
                : -globalEmergencyChargePowerW;
            return {
                deviceId: d.id,
                deviceName: d.name,
