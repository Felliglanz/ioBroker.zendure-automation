...
class RelayProtection {
    ...
    applyProtection(params) {
        ...
        // ========== DISCHARGE BLOCKED DOWNSTREAM: FREEZE STATE ==========
        // SafetyLimiter runs AFTER this and, while an active recovery lasts, always
        // clamps discharge back to 0 - so lastSetPowerW (the actually written value)
        // never leaves 0 during that whole recovery. Without this check, every single
        ...
        // though nothing is ever actually written. The moment the recovery flag briefly
        // clears (e.g. cell voltage recovers just enough), whatever this churn happened
        // to be outputting that instant gets written for real - producing a real 0/10W/
        // full-power flicker on the device instead of one clean hold-then-release.
        // Freezing here means RelayProtection always reports "still standby" while
        // blocked, so the very next real attempt starts a single, clean transition.
        if (dischargeBlocked && wantsToDischarge) {
            this.feedInCounter = 0;
            this.dischargeCounter = 0;
            this.deadbandCounter = 0;
            ...
            return {
                ...
                dischargeCounter: 0,
                deadbandCounter: 0,
                relayModified: newBatteryPowerW !== 0
            };
        }
