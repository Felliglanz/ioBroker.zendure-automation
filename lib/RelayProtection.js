 ...
 * - Relay protection (waits for measured power back within the operating deadband before switching)
...
class RelayProtection {
    ...
    applyProtection(params) {
        ...
        // Relay-safe to switch once measured power is back down near the deadband hold level -
        // tied to operatingDeadbandW (not a separate constant) so a larger configured deadband
        // doesn't leave the switch condition waiting on a tolerance smaller than what we actually
        // hold at while transitioning (see _handleTransitionToCharge/_handleTransitionToDischarge).
        // +5W flat on top: we're holding *at* operatingDeadbandW ourselves while waiting, so the
        // real device reading a couple watts of measurement noise above that (e.g. 21W against a
        // 20W deadband) must not block the switch forever - a fixed watt margin (not a percentage)
        // so it stays meaningful even at small configured deadbands (10W -> 15W, not 12.5W).
        const modeSwitchToleranceW = operatingDeadbandW + 5;
            ...
            if (this.feedInCounter < feedInDelayTicks) {
                ...
                if (currentlyDischarging) {
                    ...
                    if (Math.abs(currentBatteryPowerW) > modeSwitchToleranceW) {
                        this.adapter.log.debug(
                            `Waiting for battery power near ${operatingDeadbandW}W before relay switch (current: ${currentBatteryPowerW}W)`
                        );
                        return Math.max(holdW, newBatteryPowerW);
                    } else {
                        this.adapter.log.debug(
                            `✓ Feed-in sustained, battery at ${currentBatteryPowerW}W (~${operatingDeadbandW}W), relay safe to switch`
                        );
                    }
                } else {
        ...
        if (gridPowerW > dischargeThresholdW) {
            ...
            if (this.dischargeCounter < dischargeDelayTicks) {
                ...
                if (Math.abs(currentBatteryPowerW) > modeSwitchToleranceW) {
                    ...
                    this.adapter.log.debug(
                        `✓ Grid draw sustained, battery at ${currentBatteryPowerW}W (~${operatingDeadbandW}W), relay safe to switch`
                    );
                }
            }
        } else {
