        ...
        if (absNewPower > 0 && absNewPower < operatingDeadbandW) {
            ...
            if (this.deadbandCounter <= deadbandHoldTicks) {
                ...
                this.adapter.log.debug(
                    ...
                    `(${this.deadbandCounter}/${deadbandHoldTicks} ticks) - prevents relay switch through 0W`
                );
                protectedPowerW = minOperatingPower;
            } else {
                // Held long enough - allow transition
                this.adapter.log.debug(
                    `✓ Operating deadband RELEASED after ${this.deadbandCounter} ticks, allowing transition to ${newBatteryPowerW}W`
                );
                this.deadbandCounter = 0;
            ...
            const newSign = newBatteryPowerW < 0 ? -1 : 1;
            if (this.lastStableSign !== newSign && this.deadbandCounter > 0) {
                this.adapter.log.debug(
                    `Operating deadband: Exited to full power (${newBatteryPowerW}W), counter reset`
                );
            }
