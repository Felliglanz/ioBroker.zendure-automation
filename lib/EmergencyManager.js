...
class EmergencyManager {
    ...
    async restoreRecoveryStates() {
        ...
        const socState = await this.adapter.getStateAsync('status.socRecoveryActive');
        if (socState && socState.val === true) {
            this.inSocRecovery = true;
            this.adapter.log.info('Restored SOC recovery state from previous session');
        }

        // Restore minSoc recovery state
        const minSocState = await this.adapter.getStateAsync('status.minSocRecoveryActive');
        if (minSocState && minSocState.val === true) {
            this.inMinSocRecovery = true;
            this.adapter.log.info('Restored minSoc recovery state from previous session');
        }
    }
    ...
    async updateSocRecovery(config, batterySoc) {
        ...
        if (protectionMode !== 'soc') {
            if (this.inSocRecovery) {
                this.inSocRecovery = false;
                await this.adapter.setStateAsync('status.socRecoveryActive', false, true);
            }
            return false;
        }
    ...
    async activateSocRecovery(batterySoc, minSoc) {
        ...
        this.inSocRecovery = true;
        await this.adapter.setStateAsync('status.socRecoveryActive', true, true);
    }
    ...
    async updateMinSocRecovery(config, batterySoc, zendureMinSoc) {
        ...
        if (protectionMode !== 'voltage' || config.useZendureMinSoc === false) {
            if (this.inMinSocRecovery) {
                ...
                await this.adapter.setStateAsync('status.minSocRecoveryActive', false, true);
            }
            return false;
        }
    ...
    async activateMinSocRecovery(batterySoc, effectiveLimit) {
        ...
        this.inMinSocRecovery = true;
        await this.adapter.setStateAsync('status.minSocRecoveryActive', true, true);
    }
}
