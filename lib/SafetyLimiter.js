...
class SafetyLimiter {
    ...
    async _applySocProtection(config, emergencyManager, batterySoc) {
        ...
        if (batterySoc <= minSoc) {
            ...
            this.adapter.log.debug(
                `Battery SOC low (${batterySoc}% <= ${minSoc}%), preventing discharge`
            );
            return { blocked: true };
        }

        // Check if in SOC recovery mode
        if (emergencyManager.inSocRecovery) {
            this.adapter.log.debug('SOC recovery mode active, preventing discharge');
            return { blocked: true };
        }
