...
class DashboardServer {
    ...
    async _buildStatus() {
        ...
        if (isMulti) {
            ...
            // Overlay each device's configured Waterfill limits (native config, not a state) so
            // the dashboard can show what's actually effective per device - see issue #22.
            const configuredDevices = cfg.devices || [];
            configuredDevices.forEach((cfgDevice, index) => {
                const dev = devices[`device${index + 1}`];
                if (!dev) return;
                dev.maxChargePowerW = Number(cfgDevice.maxChargePowerW) || 0;
                dev.maxDischargePowerW = Number(cfgDevice.maxDischargePowerW) || 0;
                dev.chargeAllowed = cfgDevice.chargeAllowed !== false;
                dev.dischargeAllowed = cfgDevice.dischargeAllowed !== false;
            });
