...
class WaterfillDistributor {
    constructor() {
        this.directionState = {
            charge: { mode: 'spread', activeDeviceId: null, holdCycles: 0 },
            discharge: { mode: 'spread', activeDeviceId: null, holdCycles: 0 }
        };
    }
    ...
    distribute(totalPowerW, devices, config = {}) {
        ...
        const candidates = this.buildCandidates(devices, charging);
    ...
    buildCandidates(devices, charging) {
        return devices
            .map(device => {
                const soc = this.toFiniteNumber(device.soc, 0);
                const minSoc = this.toFiniteNumber(device.minSoc, 0);
                const maxSoc = this.toFiniteNumber(device.maxSoc, 100);
                ...
                const allowed = charging
                    ...
                    : device.dischargeAllowed !== false && soc > minSoc;
                const weight = charging ? Math.max(0, maxSoc - soc) : Math.max(0, soc - minSoc);
