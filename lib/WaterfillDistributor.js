...
class WaterfillDistributor {
    ...
    distribute(totalPowerW, devices, config = {}) {
        ...
        if (previousCandidates.length > 0) {
            ...
            // the outgoing side keeps sharing its portion by the same SOC weights
            // waterfill() already uses, so the total never exceeds magnitudeW.
            const blendFraction = 1 - (state.handoverHoldCycles / state.handoverTotalCycles);
            const activeAllocationW = Math.min(magnitudeW * blendFraction, active.limitW);
            const previousPortionW = magnitudeW * (1 - blendFraction);
