...
const MODE_TRANSITION_HOLD_CYCLES = 2;

// A PV-equipped device sitting near maxBatterySoc is about to curtail its own
// solar production (nothing left to charge into), unlike an AC-only device at
// the same SOC, which loses nothing by staying full. Nudging discharge
// priority towards it - reusing the SOC-weighted candidates already built for
// this purpose - creates headroom exactly where it's actually worth something,
// without needing to know current/forecast solar output at all: the reasoning
// holds structurally regardless of whether the sun happens to be out right now
// (issue #26). Deliberately small and narrowly scoped so it only ever breaks a
// near-tie between two already-close-to-full devices, never overrides a real
// SOC gap towards a device that actually needs to discharge more urgently -
// and it fades on its own as soon as the device drops out of the near-full
// band, so there's no lasting bias once the moment it was meant for has passed.
//
// Enter and exit use different thresholds (a proper hysteresis band, not one
// shared cutoff): a single 5%-of-maxSoc line would flip the bonus on and off
// every time SOC ticks a fraction of a percent across it, and the bonus (5)
// isn't reliably smaller than every possible waterfillSocMargin a user might
// configure - so a single-cutoff version could genuinely flap the active
// device back and forth right at that line. Armed at maxSoc-5%, only
// disarmed again below maxSoc-10% - the 5-point gap between them is dead
// zone where whatever was already decided just keeps holding, so this alone
// (not the unrelated waterfillSocMargin) is what protects against chatter.
const PV_NEAR_FULL_ENTER_MARGIN_PERCENT = 5;
const PV_NEAR_FULL_EXIT_MARGIN_PERCENT = 10;
const PV_DISCHARGE_PRIORITY_BONUS = 5;
...
class WaterfillDistributor {
    constructor(adapter = null) {
        ...
        // Per-device latch for the near-full PV discharge bonus (issue #26) -
        // deviceId -> whether it's currently armed. Needs its own persistent
        // state, independent of direction/mode, purely to implement the
        // enter/exit hysteresis band (see PV_NEAR_FULL_ENTER/EXIT_MARGIN_PERCENT).
        this.pvNearFullArmed = new Map();
    }
    ...
    buildCandidates(devices, charging, config) {
        ...
        return devices
            .map(device => {
                ...
                // Tracked every cycle regardless of charging/discharging - the latch
                // reflects the device's actual SOC trajectory, not which direction
                // happens to be requested this particular cycle - but only ever
                // applied to the discharge weight below.
                const armed = device.hasPv === true ? this.updatePvNearFullArmed(device.id, soc, maxSoc) : false;
                if (!charging && armed) {
                    weight += PV_DISCHARGE_PRIORITY_BONUS;
                }
    ...
    /**
     * Enter/exit hysteresis latch for the near-full PV discharge bonus (issue #26).
     * Arms once soc reaches maxSoc - PV_NEAR_FULL_ENTER_MARGIN_PERCENT, but only
     * disarms again once soc drops below maxSoc - PV_NEAR_FULL_EXIT_MARGIN_PERCENT -
     * the gap between those two lines is a dead zone where whatever was already
     * decided just keeps holding, so a device hovering right at one line doesn't
     * flip the bonus on and off every cycle.
     * @private
     */
    updatePvNearFullArmed(deviceId, soc, maxSoc) {
        const wasArmed = this.pvNearFullArmed.get(deviceId) === true;
        let armed = wasArmed;
        if (soc >= maxSoc - PV_NEAR_FULL_ENTER_MARGIN_PERCENT) {
            armed = true;
        } else if (soc < maxSoc - PV_NEAR_FULL_EXIT_MARGIN_PERCENT) {
            armed = false;
        }
        this.pvNearFullArmed.set(deviceId, armed);
        return armed;
    }
