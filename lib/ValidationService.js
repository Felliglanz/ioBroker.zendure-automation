    ...
    async validateSetpoint(config, actualPowerW) {
        ...
        if (withinTolerance) {
            ...
            if (this.validationRetryCount < maxRetries) {
                this.adapter.log.debug(
                    `⏳ Charge setpoint ramping (${this.validationRetryCount}/${maxRetries}): ${expectedPowerW}W (actual: ${actualPowerW}W, getting closer)`
                );
                return false; // No resend needed, device is working
            } else {
                // Ramping too slow, timeout reached
                this.adapter.log.warn(
                    `⚠️ Ramping timeout after ${maxRetries} cycles (${maxRetries * 5}s): target=${expectedPowerW}W, actual=${actualPowerW}W - device ramping too slowly, will retry`
                );
                this.pendingValidation = false;
                this.validationRetryCount = 0;
                this.previousPowerW = null;
