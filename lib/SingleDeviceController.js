...
class SingleDeviceController {
    ...
    async runCycle(config) {
        // Read early: needed by setpoint validation below (to suspend it near max SOC),
        // not just by the regulation logic further down.
        const batterySoc = await this.dataReader.getBatterySoc(config.ignoreStateFreshness);
        ...
        if (validationSource === 'packPower') {
            validationPowerW = await this.dataReader.getCurrentBatteryPowerW(config.ignoreStateFreshness);
        } else if (validationSource === 'gridInputPower') {
            validationPowerW = await this.dataReader.getGridInputPowerW(config.ignoreStateFreshness);
        }
        // If validationSource is 'none' or invalid, validationPowerW stays null (no validation)

        if (validationPowerW !== null) {
            await this.validationService.validateSetpoint(this.deviceId, config, validationPowerW, batterySoc);
        }
        // ================================================================

        // ========== READ CURRENT VALUES ==========
        const gridPowerW = await this.dataReader.getGridPowerW(config.powerMeterDp);
        const currentBatteryPowerW = await this.dataReader.getCurrentBatteryPowerW(config.ignoreStateFreshness);
        ...
        // Always checked against real battery power, independent of the configured
        // validationSource above (which governs charge validation only) - a "did we
        // actually reach idle" check has no meaningful alternative source.
        await this.validationService.validateZeroSetpoint(this.deviceId, this.deviceBasePath, config, currentBatteryPowerW);
        ...
        // I-Regulator formula (using filtered grid power). Gain defaults to 1 (full error
        // applied each cycle, original behavior) unless explicitly opted into via
        // regulatorGainEnabled - a lower value trades regulation speed for stability margin
        // against loop delay (grid meter + device response lag), see issue #30.
        const regulatorGain = config.regulatorGainEnabled && Number.isFinite(config.regulatorGain)
            ? config.regulatorGain
            : 1;
        let newBatteryPowerW = lastSetPowerW + regulatorGain * (filteredGridPowerW - targetGridPowerW);
        ...
        // Recovery flags were already refreshed for this cycle in handleEmergency() above,
        // so this reflects the same block SafetyLimiter is about to apply further down -
        // RelayProtection needs it in advance to avoid churning its deadband state on a
        // discharge/charge attempt it already knows will be vetoed (see RelayProtection.js).
        const dischargeBlocked = !config.enableDischarge ||
            this.emergencyMgr.inEmergencyRecovery ||
            this.emergencyMgr.inVoltageRecovery ||
            this.emergencyMgr.inSocRecovery ||
            this.emergencyMgr.inMinSocRecovery;
        const chargeBlocked = !config.enableCharge ||
            batterySoc >= (config.maxBatterySoc ?? 100) ||
            this.emergencyMgr.inMaxSocRecovery;
        ...
        // ========== POWER REGULATION (Hysteresis, Ramping, Limits) ==========
        // PowerRegulator's hysteresis compares against newBatteryPowerW, which already has
        // regulatorGain applied - without this scaling, a lower gain would silently inflate
        // the effective grid-error tolerance to hysteresisW/gain instead of the configured
        // hysteresisW (issue #30: gain 0.4 turned a 50W hysteresis into a 125W dead zone,
        // causing the regulator to get stuck well away from target).
        const regulationConfig = { ...config, hysteresisW: (config.hysteresisW || 50) * regulatorGain };
        ...
        // ========== WRITE TO DEVICE ==========
        // Safety-forced 0W (voltage/minSoc/SOC recovery) must bypass zero-avoidance -
        // never hold a phantom keep-alive while discharge is blocked for battery
        // protection (mirrors the excluded-device fix in MultiDeviceManager, #28).
        await this.validationService.writePowerSetpoint(
            this.deviceId,
            this.deviceBasePath,
            newBatteryPowerW,
            safetyActive ? undefined : config
        );
    ...
    async handleEmergency(config, batterySoc, minPackVoltageV) {
        ...
        await this.emergencyMgr.updateMaxSocRecovery(config, batterySoc);
        ...
        if (this.emergencyMgr.inEmergencyRecovery) {
            ...
            if (batterySoc < emergencyExitSoc) {
                ...
                // Stop charging, discharge blocked by SafetyLimiter. Bypass zero-avoidance
                // (no config passed) - this is a safety-forced 0, not routine standby (#28).
                await this.validationService.writePowerSetpoint(this.deviceId, this.deviceBasePath, 0);
        ...
        if (emergencyState.isEmergency) {
            ...
            // bypassHoldOff: responsiveness outranks flash-write avoidance here - a pending
            // post-zero grace window must not swallow the first emergency charge command
            // (mirrors the continuing-charge write above and the multi-device equivalent).
            await this.validationService.writePowerSetpoint(this.deviceId, this.deviceBasePath, emergencyChargePower, config, { bypassHoldOff: true });
            return { handled: true };
        }
