...
class SingleDeviceController {
    ...
    async runCycle(config) {
        ...
        // ========== WRITE TO DEVICE ==========
        await this.validationService.writePowerSetpoint(this.deviceId, this.deviceBasePath, newBatteryPowerW);
    ...
    async handleEmergency(config, batterySoc, minPackVoltageV) {
        ...
        if (this.emergencyMgr.inEmergencyRecovery) {
            ...
            if (batterySoc < emergencyExitSoc) {
                ...
                await this.validationService.writePowerSetpoint(this.deviceId, this.deviceBasePath, emergencyChargePower);
                return { handled: true };
            } else {
