...
class SingleDeviceController {
    ...
    async runCycle(config) {
        ...
        const deviceId = this.deviceBasePath; // Use basePath as unique device ID in single-device mode
        ...
        if (validationPowerW !== null) {
            await this.validationService.validateSetpoint(deviceId, config, validationPowerW);
        }
        // ================================================================
        ...
        const validationState = this.validationService.getDeviceState(deviceId);
        ...
        // ========== WRITE TO DEVICE ==========
        await this.validationService.writePowerSetpoint(deviceId, this.deviceBasePath, newBatteryPowerW);
    ...
    async handleEmergency(config, batterySoc, minPackVoltageV) {
        ...
        if (this.emergencyMgr.inEmergencyRecovery) {
            ...
            if (batterySoc < emergencyExitSoc) {
                ...
                await this.validationService.writePowerSetpoint(deviceId, this.deviceBasePath, emergencyChargePower);
                return { handled: true };
            } else {
                ...
                await this.validationService.writePowerSetpoint(deviceId, this.deviceBasePath, 0);
