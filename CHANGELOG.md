...
- New: writable per-device control overrides under `control.devices.<id>.*` (`maxChargePowerW`, `maxDischargePowerW`, `chargeAllowed`, `dischargeAllowed`), overlaid on top of the admin-config values for the running session - the same pattern already used for the existing global `control.*` states.
...
- Neu: schreibbare Pro-Gerät-Steuerungs-Overrides unter `control.devices.<id>.*` (`maxChargePowerW`, `maxDischargePowerW`, `chargeAllowed`, `dischargeAllowed`), die für die laufende Session über die Admin-Config-Werte gelegt werden - dasselbe Muster wie bei den bestehenden globalen `control.*`-States.
