...
### English
- **PV charge headroom for multi-device Waterfill (issue #26):** opt-in `hasPv` flag per device treats `maxChargePowerW` as the combined PV+AC ceiling instead of an AC-only limit. Live `solarInputPower` is subtracted each cycle, and the resulting shortfall against the requested AC-side charge power is redistributed to other devices within the same cycle instead of stalling on repeated setpoint-validation retries. Also forces `gridInputPower` as the validation source for PV devices so the setpoint check compares against the right measurement.
- **Dashboard: per-device PV vs. battery share:** multi-device cards now show "☀ PV + 🔋 Battery" separately (no computed sum) whenever that device's own solar is actually producing, instead of only ever showing battery power - previously the card total never reconciled with the flow diagram whenever PV covered part of the output directly.
- **Single→spread transition blending (issue #40):** a load spike crossing the spread threshold used to instantly commit both devices to their final SOC-weighted split, even though the newly joining device can't actually deliver its share yet (relay closing, inverter start) - the I-Regulator read the shortfall as grid error, overcorrected, then overshot once the joiner caught up (visible as a grid-power spike followed by a feed-in dip). Now mirrors the existing spread→single concentrate blend: the already-running device keeps covering as much of the load as it can while the joiner ramps in, over 4 cycles (raised from an initial 2, since automation cycles fire on grid changes too and aren't reliably ~5s apart).
- **RelayProtection/ValidationService: removed a measured-power gate from charge↔discharge transitions (issue #40):** the transition used to also wait for measured battery power to converge near `operatingDeadbandW` before switching, on top of the existing tick-based sustained-condition counters. A PV-equipped device can keep charging/discharging from its own solar regardless of that small AC-side hold, so convergence could never happen - the adapter got stuck refusing to switch for 6+ minutes while ignoring several kW of sustained grid draw. The tick-based hold is itself already the settling time; once the delay ticks elapse, it now commits to the switch instead of also waiting on a measurement that may never confirm. Fixed the same root cause in `ValidationService`: a 0W setpoint confirmed via `outputLimit`/`inputLimit` but never via measured power (same PV scenario) now resends the literal 0 once past the retry threshold instead of every single cycle indefinitely.
- **Fixed a stale zero-avoidance direction causing phantom discharge pulses (issue #38):** `ValidationService` tracked its own `lastDirection` for the zero-avoidance keep-alive, separate from `lastWrittenLimit`. A safety-forced 0W (e.g. `SafetyLimiter` blocking discharge during SOC recovery) is written through a bypass path that never updated `lastDirection`, so it stayed frozen at whatever direction was active before the block - the next legitimate 0W then got laundered into a real, non-zero pulse in that stale direction, straight into a device `SafetyLimiter` was actively blocking. Direction is now derived fresh from `lastWrittenLimit` every time, and a request for 0 when it's already a genuine 0 skips the keep-alive dance entirely.
- **Per-device state-freshness bypass switch (issue #39/#16):** cloud-MQTT-only devices (e.g. Hyper 2000, no ZenSDK) may only republish `packPower`/`gridInputPower` on change, so a legitimately unchanged value could trip the frozen-state watchdog and wrongly exclude the device from a cycle. Adds an explicit opt-in checkbox per device (single- and multi-device mode) instead of a heuristic.

### Deutsch
- **PV-Ladefreigabe für Multi-Device-Waterfill (Issue #26):** Optionales `hasPv`-Flag pro Gerät behandelt `maxChargePowerW` als kombinierte PV+AC-Obergrenze statt als reines AC-Limit. Die aktuelle `solarInputPower` wird jeden Zyklus abgezogen, und die daraus entstehende Differenz zur angeforderten AC-Ladeleistung wird im selben Zyklus auf die anderen Geräte verteilt, statt in wiederholten Sollwert-Validierungsfehlern zu enden. Erzwingt außerdem `gridInputPower` als Validierungsquelle für PV-Geräte, damit die Sollwertprüfung gegen den richtigen Messwert läuft.
- **Dashboard: PV- vs. Batterie-Anteil pro Gerät:** Multi-Device-Karten zeigen jetzt getrennt "☀ PV + 🔋 Batterie" an (keine berechnete Summe), sobald das jeweilige Gerät tatsächlich eigene Solarleistung liefert, statt nur die Batterieleistung - bisher stimmte die Kartensumme nie mit dem Flussdiagramm überein, sobald PV einen Teil der Ausgabe direkt deckte.
- **Single→Spread-Übergang mit Blending (Issue #40):** Eine Lastspitze über der Spread-Schwelle hat bisher beide Geräte sofort auf ihre finale SOC-gewichtete Aufteilung festgelegt, obwohl das neu hinzukommende Gerät seinen Anteil noch gar nicht liefern kann (Relais schließt, Wechselrichter startet) - der I-Regler hat die Differenz als Netzfehler gelesen, überkorrigiert und beim Aufholen des neuen Geräts dann überschossen (sichtbar als Netzleistungsspitze gefolgt von einem Einspeise-Einbruch). Läuft jetzt analog zum bestehenden Spread→Single-Übergang: Das bereits laufende Gerät deckt weiterhin so viel wie möglich ab, während das neue Gerät eingeblendet wird, über 4 Zyklen (angehoben von anfänglich 2, da Regelzyklen auch bei Netzänderungen auslösen und nicht zuverlässig ~5s auseinanderliegen).
- **RelayProtection/ValidationService: Messwert-Gate beim Wechsel Laden↔Entladen entfernt (Issue #40):** Der Übergang wartete bisher zusätzlich zu den bestehenden taktbasierten Zählern darauf, dass die gemessene Batterieleistung nahe `operatingDeadbandW` konvergiert, bevor umgeschaltet wird. Ein Gerät mit eigenem Solareingang kann unabhängig von dieser kleinen AC-seitigen Haltung weiter über PV laden/entladen, wodurch diese Konvergenz nie eintreten konnte - der Adapter hat die Umschaltung dadurch über 6 Minuten verweigert und dabei mehrere kW anhaltenden Netzbezug ignoriert. Die taktbasierte Haltezeit ist selbst schon die Einschwingzeit; nach Ablauf der Verzögerungstakte wird jetzt direkt umgeschaltet, statt zusätzlich auf einen Messwert zu warten, der sich nie bestätigen könnte. Denselben Grundfehler in `ValidationService` behoben: Ein über `outputLimit`/`inputLimit` bestätigtes, aber messtechnisch nie bestätigtes 0W (gleiches PV-Szenario) wird jetzt nach Erreichen der Retry-Schwelle nur noch einmal statt endlos jeden Zyklus erneut gesendet.
- **Veraltete Zero-Avoidance-Richtung behoben, die Phantom-Entlade-Impulse verursachte (Issue #38):** `ValidationService` führte eine eigene `lastDirection` für den Zero-Avoidance-Keep-Alive, getrennt von `lastWrittenLimit`. Ein sicherheitsbedingtes 0W (z.B. `SafetyLimiter` blockiert Entladen während SOC-Recovery) wird über einen Bypass-Pfad geschrieben, der `lastDirection` nie aktualisierte - dieser blieb also auf der zuletzt aktiven Richtung vor der Sperre eingefroren. Das nächste legitime 0W wurde dadurch in einen echten, von Null verschiedenen Impuls in genau dieser veralteten Richtung umgewandelt - direkt in ein Gerät, das `SafetyLimiter` aktiv blockierte. Die Richtung wird jetzt bei jedem Aufruf frisch aus `lastWrittenLimit` abgeleitet, und eine Anfrage für 0 bei bereits echtem 0 überspringt den Keep-Alive-Tanz komplett.
- **Pro-Gerät-Schalter für State-Freshness-Prüfung (Issue #39/#16):** Reine Cloud-MQTT-Geräte (z.B. Hyper 2000, ohne ZenSDK) senden `packPower`/`gridInputPower` teils nur bei Änderung neu, wodurch ein legitim unveränderter Wert den Frozen-State-Watchdog auslösen und das Gerät fälschlich aus einem Zyklus ausschließen konnte. Fügt eine explizite Opt-in-Checkbox pro Gerät hinzu (Single- und Multi-Device-Modus) statt einer Heuristik.

## v1.1.3 (2026-08-29)

### English
- **Optional InfluxDB v2 export:** periodic snapshot of `telemetry.*`/`status.*` to your own InfluxDB v2 bucket, dependency-free (hand-rolled line protocol). Device tagging is consistent across single- and multi-device mode, so the bucket schema is identical regardless of mode. The token is stored encrypted/masked. Adds `telemetry.housePowerW`/`pvPowerW` (previously only derived transiently for the live dashboard) so both daily Wh totals and wave-shaped power curves are available to export.
- **New dashboard "Historie" view:** shown once InfluxDB export and its own dashboard checkbox are both enabled, it reads data back from Influx via a server-side Flux query proxy - the Influx token never reaches the browser, and query inputs are validated against strict patterns to rule out Flux injection. Field picker shows curated, human-readable labels (raw strings/booleans and internal-only counters are filtered out). Supports up to two independently-scaled overlaid fields, time-range presets (including a new "Heute" = since local midnight) plus a custom range, and a hover/touch crosshair tooltip. The existing "Telemetrie" entry is renamed "Tagesansicht" to make room for it.
- Fixed the History chart's baseline for signed metrics (grid/battery power): it now uses a symmetric, zero-centered scale with the area fill anchored to the zero line, matching the small per-card graphs, instead of always filling down to the plot's bottom edge.
- Fixed stacking History-chart tooltips: switching field/range while hovering used to leave the previous tooltip and pointer listeners behind instead of replacing them, so several stale ones could overlap.
- **RelayProtection livelock fix (charge↔discharge transitions):** while waiting for measured battery power to settle before a relay switch, RelayProtection asked for a literal 0W. With `avoidZeroSetpoint` enabled, that 0 got laundered by `ValidationService` into a non-zero keep-alive in the *old* direction, which read back next cycle and restarted the transition from scratch - an unbounded livelock where no command ever reached the device (real-world case: stuck discharging for hours despite sustained feed-in, only a restart recovered). It now holds at ±`operatingDeadbandW` instead of a literal 0 during a transition, with a small added safety margin so sensor noise right at that boundary can't block the switch indefinitely.
- **New 0W setpoint validation:** literal 0W writes (safety-forced or the eventual zero-avoidance commit) are now cross-checked against nograx's own stored `outputLimit`/`inputLimit` state, independent of the existing charge-setpoint validation. Catches a nograx-side desync where the command was accepted but the underlying property write silently failed to apply - observed case: `outputLimit` stuck at 10W after a restart-timing race, device kept discharging in 10W blips despite a confirmed 0W request. A genuinely new setpoint (e.g. an emergency charge) always writes immediately regardless.

### Deutsch
- **Optionaler InfluxDB-v2-Export:** periodischer Snapshot von `telemetry.*`/`status.*` in einen eigenen InfluxDB-v2-Bucket, ohne zusätzliche Abhängigkeit (selbst implementiertes Line-Protocol). Die Geräte-Kennzeichnung ist in Single- und Multi-Device-Modus identisch, das Bucket-Schema ist also modusunabhängig gleich. Der Token wird verschlüsselt/maskiert gespeichert. Neu: `telemetry.housePowerW`/`pvPowerW` (bisher nur transient fürs Live-Dashboard berechnet), damit sowohl Tages-Wh-Summen als auch Leistungskurven exportiert werden können.
- **Neue Dashboard-Ansicht "Historie":** erscheint, sobald InfluxDB-Export und die zugehörige Dashboard-Checkbox aktiviert sind, und liest die Daten über einen serverseitigen Flux-Query-Proxy zurück - der Influx-Token erreicht den Browser nie, Query-Eingaben werden gegen strikte Muster geprüft, um Flux-Injection auszuschließen. Die Feldauswahl zeigt kuratierte, verständliche Bezeichnungen (rohe Strings/Booleans und rein interne Zähler werden ausgeblendet). Bis zu zwei unabhängig skalierte, überlagerte Felder, Zeitraum-Presets (inkl. neuem "Heute" = seit lokaler Mitternacht) oder frei wählbarer Bereich, sowie ein Hover-/Touch-Fadenkreuz-Tooltip. Der bisherige Eintrag "Telemetrie" heißt jetzt "Tagesansicht", um Platz zu schaffen.
- Die Basislinie des Historie-Charts für vorzeichenbehaftete Metriken (Netz/Batterie) behoben: jetzt eine symmetrische, um 0 zentrierte Skala mit auf der Nulllinie verankerter Flächenfüllung, analog zu den kleinen Karten-Graphen, statt immer bis zum unteren Rand zu füllen.
- Ein Stapel-Problem bei Historie-Chart-Tooltips behoben: Beim Wechsel von Feld/Zeitraum während des Hoverns blieb der vorherige Tooltip samt Pointer-Listenern bestehen statt ersetzt zu werden, wodurch sich mehrere veraltete Tooltips überlagern konnten.
- **RelayProtection-Livelock-Fix (Wechsel Laden↔Entladen):** Beim Warten auf eine eingeschwungene Batterieleistung vor einem Relaisschaltvorgang forderte RelayProtection ein echtes 0W an. Bei aktiviertem `avoidZeroSetpoint` wurde diese 0 von `ValidationService` in einen Keep-Alive-Wert in der *alten* Richtung umgewandelt, der im nächsten Zyklus zurückgelesen wurde und den Übergang komplett neu startete - ein unbegrenzter Livelock, bei dem nie ein Befehl das Gerät erreichte (realer Fall: stundenlang festhängend im Entladen trotz anhaltender Einspeisung, nur ein Neustart half). Jetzt wird während eines Übergangs bei ±`operatingDeadbandW` gehalten statt bei echten 0, mit einer kleinen zusätzlichen Sicherheitsmarge, damit Messrauschen genau an dieser Grenze den Schaltvorgang nicht dauerhaft blockiert.
- **Neue 0W-Sollwert-Validierung:** Echte 0W-Schreibvorgänge (sicherheitsbedingt oder der spätere Zero-Avoidance-Commit) werden jetzt zusätzlich, unabhängig von der bestehenden Ladesollwert-Validierung, gegen nograx's eigenen gespeicherten `outputLimit`/`inputLimit`-Zustand geprüft. Erkennt eine geräteseitige Desynchronisation, bei der der Befehl zwar angenommen wurde, der zugrundeliegende Property-Write aber unbemerkt fehlschlug - beobachteter Fall: `outputLimit` blieb nach einem Timing-Problem beim Neustart bei 10W hängen, das Gerät entlud weiter in 10W-Blips trotz bestätigter 0W-Anfrage. Ein echter neuer Sollwert (z.B. Notladung) wird davon unabhängig immer sofort geschrieben.
